...
# Linux full boot (약 60~90분) ⚠️ run.sh linux 미검증
./build.sh buildroot
./run.sh linux
```
...
Buildroot (aarch64 / ATF + U-Boot + Linux):
  ...
  buildroot-uboot        U-Boot만 재빌드                  ⚠️ 미검증
  buildroot-kernel       Linux 커널만 재빌드               ⚠️ 미검증
...
./build.sh buildroot-kernel                      # ⚠️ 미검증
...
Targets:
  linux     ATF → U-Boot → Linux 부팅 (aarch64)   ⚠️ Docker 경유 미검증
...
```bash
./run.sh freertos                  # ✅ 검증됨
./run.sh linux                     # ⚠️ Docker 경유 미검증
./run.sh linux --smp 2 --mem 2048M # ⚠️ 미검증
...
> ⚠️ 위 부팅 체인은 Docker 도입 전 호스트 환경에서 검증됨.
> Docker 경유 `./run.sh linux` 실행은 미검증.
...
| 항목 | 상태 | 비고 |
|---|---|---|
| `./run.sh linux` (Docker 경유) | ⚠️ 미검증 | buildroot 빌드만 확인 |
| Docker 자동설치 자동 트리거 | ⚠️ 미검증 | 수동 설치 후 동작 확인 |
| `run.sh` Linux 옵션 (`--smp`, `--mem` 등) | ⚠️ 미검증 | 구현 완료 |
| `run.sh --gdb` (FreeRTOS/Linux) | ⚠️ 미검증 | 구현 완료 |
| `build.sh buildroot-uboot/kernel` 부분 재빌드 | ⚠️ 미검증 | 구현 완료 |
