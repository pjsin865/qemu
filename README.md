...
| 타겟 | 설명 | 검증 |
|------|------|:----:|
| `freertos-riscv` | FreeRTOS CLI 데모 빌드 (rv32imac, xPack GCC 14.x, `freertos_riscv_cli/`) | ✅ |
...
| `freertos-riscv` | FreeRTOS CLI 쉘 (rv32 / QEMU virt) | ✅ |
...
### FreeRTOS CLI 명령어 (RISC-V rv32) ✅

QEMU 실행 후 `$ ` 프롬프트가 나타나면 동일한 명령어를 사용할 수 있습니다:

| 명령어 | 설명 | 검증 |
|---|---|---|
| `help` | 사용 가능한 명령어 목록 | ✅ |
| `task-stats` | FreeRTOS 태스크 목록 및 상태 | ✅ |
| `echo <text>` | 텍스트 출력 (첫 번째 단어만) | ✅ |
| `version` | FreeRTOS 커널 버전 및 플랫폼 정보 | ✅ |
| `uptime` | 시스템 가동 시간 (ticks 포함) | ✅ |
| `free-heap` | 남은 FreeRTOS 힙 크기 | ✅ |
...
$ version
FreeRTOS V11.1.0+ | RISC-V rv32 QEMU virt | NS16550A UART
$ task-stats
Name            State  Pri  Stack  Num
--------------------------------------
CLI            	X	1	802	1
...
Tmr Svc        	B	6	205	3
$ free-heap
Free heap: 57616 bytes
$ uptime
Uptime: 00:00:02  (ticks: 2491)
```
...
### FreeRTOS CLI 아키텍처 (rv32 / QEMU virt) ✅

```
QEMU virt machine (-machine virt -bios none -smp 4)
     │
     ▼
start.S / vector.S     — FreeRTOS RISC-V 부트 코드 (Demo에서 재사용)
     │
     ▼
main()
  ├── csrw mtvec, freertos_risc_v_trap_handler  — 트랩 핸들러 설정
  ├── uart_init()              — NS16550A UART0 (0x10000000) 초기화
  ├── vRegisterCLICommands()  — CLI 명령어 5개 등록
  ├── xTaskCreate(vCLITask)   — CLI 태스크 생성 (스택 1KB)
  └── vTaskStartScheduler()  — FreeRTOS 스케줄러 시작
         │
         ▼
    vCLITask (non-blocking loop)
...
**ARM FreeRTOS CLI vs RISC-V FreeRTOS CLI 비교:**

| 항목 | Cortex-M3 (ARM) | rv32 (RISC-V) |
|------|-----------------|---------------|
| 소스 | `freertos_cli/` | `freertos_riscv_cli/` |
| 데모 유형 | 인터랙티브 CLI 쉘 | 인터랙티브 CLI 쉘 |
...
| 툴체인 | arm-none-eabi-gcc (Ubuntu apt) | xPack riscv-none-elf-gcc 14.x |
| 힙 | ~192KB (large MPU demo) | ~57KB (커스텀 configTOTAL_HEAP_SIZE=64K) |

**NS16550A UART 레지스터 (base: 0x10000000):**

| 레지스터 | 오프셋 | 설명 |
|---|---|---|
| RBR/THR | +0x00 | 수신/송신 데이터 |
| IER | +0x01 | 인터럽트 활성화 |
| FCR | +0x02 | FIFO 제어 |
| LCR | +0x03 | 라인 제어 (8N1 설정) |
| LSR | +0x05 | bit0=DR(RX ready), bit5=THRE(TX ready) |
...
├── freertos_cli/             # FreeRTOS CLI 소스 — Cortex-M3 (git tracked)
...
│   ├── mps2_m3.ld            # 링커 스크립트
│   └── Makefile
├── freertos_riscv_cli/       # FreeRTOS CLI 소스 — rv32 (git tracked)
│   ├── FreeRTOSConfig.h      # FreeRTOS 커널 설정 (CLINT 주소, CLI 버퍼 크기)
│   ├── main.c                # CLI 태스크 (MTVEC 설정, non-blocking UART 루프)
│   ├── riscv-virt.h / .c     # RISC-V 시스템 지원 + FreeRTOS 훅
│   ├── uart.h / uart.c       # NS16550A UART 드라이버 (TX+RX)
│   ├── cli_commands.h/c      # CLI 명령어 구현
│   └── Makefile              # start.S/vector.S/fake_rom.lds는 FreeRTOS Demo에서 참조
...
### v1.8.0 — RISC-V FreeRTOS CLI 추가 ✅

**추가:**
- RISC-V FreeRTOS 타겟을 Blinky 데모에서 **인터랙티브 CLI 쉘**로 업그레이드
  - `freertos_riscv_cli/` 신규 디렉토리 (Cortex-M3 `freertos_cli/`와 동일한 구조)
  - NS16550A UART TX+RX 드라이버 (`uart_getchar_nonblock()` 구현)
  - FreeRTOS+CLI 연동: `help`, `task-stats`, `version`, `uptime`, `free-heap`, `echo`
  - start.S / vector.S / fake_rom.lds는 `FreeRTOS/Demo/RISC-V-Qemu-virt_GCC/`에서 재사용
  - `configCOMMAND_INT_MAX_OUTPUT_SIZE = 512` FreeRTOSConfig.h에 추가
- `build.sh` `FREERTOS_RISCV_CLI_DIR` 변수로 변경 (이전: `FREERTOS_RISCV_DEMO_DIR`)

**검증 환경: Ubuntu 22.04 LTS, Docker Engine 29.4.3**

| 항목 | 결과 |
|------|------|
| `./build.sh freertos-riscv` (rv32imac_zicsr, xPack GCC 14.2.0) | ✅ |
| `./run.sh freertos-riscv` (`$ ` 프롬프트 확인) | ✅ |
| `help` 명령어 (6개 명령어 목록) | ✅ |
| `task-stats` (CLI / IDLE / Tmr Svc 태스크) | ✅ |
| `version` / `uptime` / `free-heap` / `echo` | ✅ |
| 백스페이스 처리 (`\b \b`) | ✅ |
